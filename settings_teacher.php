...
<html>
    ...
    <body>
        ...
        <div class="row"> <!---  шапка  --->
			...
			<div class="col-sm-2">
				<a href="settings_teacher.php"><button class="btn btn-outline-secondary col-sm-12 active" type="button" aria-expanded="false">
                    Викладачі
                </button></a>
			</div>
			<div class="col-sm-2">
			...
			<div class="col-sm-6">
				<table class="table">
					...
					<tr>
									...
									<div class="modal-dialog modal-dialog-centered">
										<div class="modal-content">
											<div class="modal-header">
												<h5 class="modal-title" id="staticBackdropLabel">Додати викладача</h5>
												<button type="button" class="btn-close text-center" data-bs-dismiss="modal" aria-label="Close"></button>
											</div>
											<div class="modal-body">
												<form method="POST" action="settings_teacher.php">
													<div class="form-floating mb-3">
														<input type="text" class="form-control" name="name" id="floatingInput" placeholder="ПІБ">
						...
						<?php 
						//   
							$sql="SELECT * FROM `teachers` WHERE `id`!=0 AND `id`!=1 AND `id`!=2 ORDER BY `name` ASC";
							$res=mysqli_query($connect,$sql);
							$n = 1;
							while($result=mysqli_fetch_array($res)){?>
							<tr>
										...
										<div class="modal-body">
											<form method="POST" action="settings_teacher.php">
												<div class="form-floating mb-3">
													...
													<input type="text" class="form-control" name="name" id="floatingInput" value="<?php echo $result['name'] ?>" placeholder="ПІБ">
													<label for="floatingInput">ПІБ</label>
												</div>
												<br>
												<div class="form-floating text-center">
...
<?php 
	if(isset($_POST['new'])){
		//name
		$sql = "INSERT INTO `teachers` (`name`) VALUES ('".$_POST['name']."')";
		$res=mysqli_query($connect,$sql);
		?>
            <script>document.location.href="settings_teacher.php"</script>
        <?php
	}
	if(isset($_POST['save'])){
		//id, name
		$sql = "UPDATE `teachers` SET `name`='".$_POST['name']."' WHERE `id`='".$_POST['id']."'";
		$res=mysqli_query($connect,$sql);
		?>
            <script>document.location.href="settings_teacher.php"</script>
        <?php
	...
	if(isset($_POST['delete'])){
		//id, name
		$sql = "DELETE FROM `teachers` WHERE `id`='".$_POST['id']."'";
		$res=mysqli_query($connect,$sql);
		?>
            <script>document.location.href="settings_teacher.php"</script>
        <?php
